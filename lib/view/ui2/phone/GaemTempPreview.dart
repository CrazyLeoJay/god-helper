    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text("神职：${_roleConfig.gods.map((e) => e.nickname).toList(growable: false)}"),
          if (_roleConfig.citizenCount > 0)
            Text("村民：${_roleConfig.citizen.map((e) => e.nickname).toList(growable: false)}"),
          Text("狼职：${_roleConfig.wolfs.map((e) => e.nickname).toList(growable: false)}"),
          Text("三方：${_roleConfig.thirds.map((e) => e.nickname).toList(growable: false)}"),
